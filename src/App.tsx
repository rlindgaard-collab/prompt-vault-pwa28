...
function capitalizeName(str: string) {
  if (!str) return ''
  const t = str.trim()
  return t.replace(/^([a-zæøå])/i, (m) => m.toUpperCase())
}
            ...
            <nav className="space-y-4">
              ...
              <div className="space-y-2">
                {current?.sections.map(sec => {
                  ...
                  return (
                    <div key={sec.section}>
                      <div className="text-sm font-semibold text-ink dark:text-white">{sec.section}</div>
          ...
          {isSearching ? (
            ...
            <>
              ...
              {current?.sections.map((s) => {
                ...
                return (
                  <section key={s.section} className="space-y-4">
                    ...
                    {s.categories.map((c) => {
                      ...
                      return (
                        <div key={c.category} id={anchorId} className="space-y-3">
                          <h3 className="text-sm text-slate-500 dark:text-slate-400">{capitalizeName(c.category)}</h3>
                          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
                            {c.prompts.map((p, i) => (
